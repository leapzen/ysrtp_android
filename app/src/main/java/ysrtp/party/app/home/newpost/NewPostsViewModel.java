...
public class NewPostsViewModel extends AndroidViewModel implements ServerResponseListener {

    @SuppressLint("StaticFieldLeak")
    private Context context;
    private MutableLiveData<NewPostsModel> newPostsModelLiveData = new MutableLiveData<>();
    ...
    public NewPostsViewModel(@NonNull Application application) {
        super(application);
    ...
    public void getNewPosts() {
        RequestModel newPostsModel = new RequestModel();
        newPostsModel.setURL(Constants.getInstance().getNewPostsUrl());
        newPostsModel.setRequestType(newPostsModel.GET);

        ServerRequest newPostsRequest = new ServerRequest(context);
        newPostsRequest.sendRequest(newPostsModel,this,Constants.NEW_POSTS);
    }
    ...
    public void getResponse(ResponseModel responseModel, int type) {
        if(responseModel.isSuccess()){
            if(type == Constants.NEW_POSTS && responseModel.getStatusCode() == 200){
                NewPostsModel newPostsModel = new Gson().fromJson(responseModel.getPayload(),NewPostsModel.class);
                NewPostsModel.setNewPostsInstance(newPostsModel);
                newPostsModelLiveData.postValue(NewPostsModel.getNewPostsInstance());
    ...
    MutableLiveData<NewPostsModel> getNewPostsModelLiveData() {
        return newPostsModelLiveData;
    }
